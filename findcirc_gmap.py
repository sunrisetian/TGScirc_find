...
def exactReads(filename):
    ...
    df = df[df.duplicated(subset=9, keep=False)]  # 去除只有单一比对结果的reads
    df = df.drop_duplicates(subset=[9, 13, 0, 11, 12], keep='first')  # 去除完全重复的全长比对reads
    return df
...
def getCircInformation(df,output_path):
        ...
        for i in range(len(index_list) - 1):
            ...
            if little_df.iloc[i, 13] == little_df.iloc[i + 1, 13]:
                ...
                if (little_df.iloc[i, 8] == '+') and (little_df.iloc[i + 1, 8] == '+') and \
                    ...
                    #res_list.append(list(little_df.iloc[i, [11, 12, 15, 16]]))
                    #res_list.append(list(little_df.iloc[i + 1, [11, 12, 15, 16]]))
    ...
    filepath = '%s_%s_gamp' % (output_path,chr_name)
